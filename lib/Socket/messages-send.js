...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if ((0, messages_1.normalizeMessageContent)(message)?.pinInChatMessage) {
            ...
            if (isGroup || isStatus) {
                ...
                for (const { user, device } of devices) {
                    const jid = (0, WABinary_1.jidEncode)(user, groupData?.addressingMode === 'lid' ? 'lid' : 's.whatsapp.net', device);
                ...
                for (const { user, device } of devices) {
                    ...
                    const jid = (0, WABinary_1.jidEncode)(isMe && isLid ? authState.creds?.me?.lid.split(':')[0] || user : user, isLid ? 'lid' : 's.whatsapp.net', device);
                    if (isMe) {
                        meJids.push(jid);
                    }
                    else {
            ...
            const contentType = (0, messages_1.getContentType)((0, messages_1.normalizeMessageContent)(message));
            if (((0, WABinary_1.isJidGroup)(destinationJid) || (0, WABinary_1.isJidUser)(destinationJid)) &&
