...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (mediaType === 'audio' && uploadData.ptt === true && uploadData.mimetype !== 'audio/ogg; codecs=opus') {
        try {
            const { convertAudioToOpus } = await Promise.resolve().then(() => __importStar(require('./audio-converter')));
    ...
    const { mediaKey, encFilePath, originalFilePath, fileEncSha256, fileSha256, fileLength } = await (0, messages_media_1.encryptedStream)(processedMedia, options.mediaTypeOverride || mediaType, {
        logger,
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('buttons' in message || 'templateButtons' in message) {
        const { convertLegacyButtons } = await Promise.resolve().then(() => __importStar(require('./messages-compat')));
...
const generateWAMessageFromContent = (jid, message, options) => {
    ...
    if (quoted && !(0, WABinary_1.isJidNewsletter)(jid)) {
        ...
        quotedMsg = index_js_1.proto.Message.fromObject({ [msgType]: quotedMsg[msgType] });
